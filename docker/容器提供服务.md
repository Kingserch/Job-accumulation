...
```
2.nginx服务
用户可对系统内核参数做一些调整，nginx服务器内核优化参数
```
...
```
3.Jetty
Jetty是一个开源的servlet容器，以其高效，小巧，可嵌入式，基于java的web内容(如jsp和servlet)提供运行环境



4.Lamp
```
doker run -p 80:80 -it linode/lamp /bin/bash
service apache2 start                                                                         
service mysql start  
```
![](https://github.com/Kingserch/Job-accumulation/blob/Docker/images/lamp.png)
+ ### Jenkins和GitLab
`docker run -p 8080 -p 5000:5000 -v /home/tools:/var/jenkins_home jenkins #数据在容器的工作目录是/var/jenkins_home，-v把宿主机的目录映射到容器中`
```
 docker run --detach \
...
```
1.mysql
[root@42-m ~]# docker run --name Imysql -e MYSQL_ROOT_PASSWORD=root -d mysql:5.7
2.MongoDB
[root@42-m ~]# docker run --name mongo-container -d mongo
3.Redis,可以用作数据库，缓存，和消息中间件
`Redis支持的数据结构:String，hash(散列)，list(列表)set(集合)sorted set(有序集合)，redis同时支持replication，LUA脚本LRU驱动事件，通哨兵机制和集群机制提供高可用性`
```
